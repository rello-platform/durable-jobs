...
export async function advanceProgress(args: {
  ...
  where: Record<string, unknown>; // { batchId } etc.
  increments: Record<string, number>; // { committedCount: 12, errorCount: 1 }
  logPrefix?: string;
}): Promise<void> {
  ...
  try {
    ...
    console.warn(
      `${prefix} advance skipped for ${JSON.stringify(args.where)} (no progress row?): ${
 ...
 */
export function advisoryLockKey(anchor: string): number {
  let key = 0;
 ...
 */
export async function mergeProgressMap<TEntry>(args: {
  tx: ProgressTxClient;
  lockNamespace: number; // consumer-owned advisory namespace (HH used 4242)
  lockKey: number; // stable 32-bit hash of the anchor (use advisoryLockKey)
  ...
  try {
    ...
    const next: Record<string, TEntry> = { ...current };
    ...
    console.error(
      `${prefix} merge failed for mapKey "${args.mapKey}" (will self-heal on reconcile): ${
        err instanceof Error ? err.message : String(err)
      }`,
    );
  }
