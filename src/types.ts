...
export type BulkOpStatus =
  | "PENDING" // intent persisted; awaiting drain
  | "PROCESSING" // a runner claimed it; work in flight
  | "WAITING" // claimed but a cross-instance gate is unmet — released for retry (opt-in)
  | "COMPLETED" // terminal success
  | "EMPTY" // terminal no-op (nothing to do after the in-drain filter)
  | "FAILED" // exhausted transient retries — operator-visible, DLQ written
  | "DEAD_LETTER"; // must-never-drop row exhausted retries — surfaced, never silently lost
...
export const TERMINAL_STATUSES = [
  ...
  "FAILED",
  "DEAD_LETTER",
] as const satisfies readonly BulkOpStatus[];
...
export function isTerminalStatus(s: BulkOpStatus): boolean {
  return (TERMINAL_STATUSES as readonly string[]).includes(s);
}

/** True iff `s` is claimable by a drain. */
export function isClaimableStatus(s: BulkOpStatus): boolean {
  return (CLAIMABLE_STATUSES as readonly string[]).includes(s);
}
...
export interface BulkOpDelegate<TRow = BulkOpIntent> {
  // METHOD-syntax members below are checked bivariantly by TS, so a real
  // (narrower) Prisma arg type — e.g. `ModelWhereUniqueInput` for `where` — is
  // assignable to our permissive `Record<string,unknown>` shape. The return
  // types are `PromiseLike` (not `Promise`) so Prisma's thenable
  // `Prisma__ModelClient<T>` — which is awaitable but NOT a real `Promise` — is
  // assignable. Together these let `prisma.<model>` fit without `as unknown as`.
  findUnique(args: {
  ...
  findMany(args: {
    ...
    take?: number;
    select?: Record<string, boolean>;
  }): PromiseLike<Array<{ id: string } & Partial<TRow>>>;
  upsert(args: {
    where: Record<string, unknown>;
  ...
  create(args: { data: Record<string, unknown> }): PromiseLike<TRow>;
}
 ...
 */
export function asBulkOpDelegate<TRow = BulkOpIntent>(
  delegate: unknown,
): BulkOpDelegate<TRow> {
  return delegate as BulkOpDelegate<TRow>;
}
