 ...
 */
export async function drainBulkOp(args) {
    ...
    if (args.onClaim) {
        try {
            ...
            console.error(`${prefix} onClaim hook threw for intent ${intent.id} (audit not blocked): ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    if (args.gate) {
        ...
        try {
            ...
            console.error(`${prefix} gate threw for intent ${intent.id}; releasing to WAITING: ${err instanceof Error ? err.message : String(err)}`);
            unmet = true;
        }
        if (unmet) {
            if (claim) {
    ...
    let items;
    try {
        items = args.selectItems(intent.payload) ?? [];
    }
    catch (err) {
    ...
    for (let start = 0; start < items.length; start += chunkSize) {
        ...
        const settled = await Promise.allSettled(chunk.map((item, i) => args.processItem(item, start + i, {
            intent,
        ...
        if (args.onChunk) {
            try {
                ...
                console.error(`${prefix} onChunk hook threw for intent ${intent.id} (progress will self-heal): ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    }
...
async function finalizeFailure(p) {
    ...
    const decision = classifyFailure({
        ...
        maxAttemptsPermanent: args.maxAttemptsPermanent ?? intent.maxAttemptsPermanent,
    ...
    console.warn(`${prefix} ${verb} intent ${intent.id} after exhaustion (${decision.counter}=${decision.nextCount}): ${message.slice(0, 200)}`);
...
export async function drainBulkOpBatch(args) {
    ...
    if (candidates.length === 0)
        return tally; // count-first idle skip
    for (const c of candidates) {
        try {
            ...
            tally.pending++;
            console.error(`${prefix} drainOne threw for intent ${c.id}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    return tally;
}
